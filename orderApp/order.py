...
class SeverApplication(object):
    def __init__(self, instructparser):
        self.ss = socket(AF_INET, SOCK_STREAM)
        self.port = ('', 8889)
        self.inparser = instructparser
    ...
    def connect(self):
        ...
        while True:
            ...
            if not cs:
                break
            self.inparser.parse.print_help()
            f = cs.makefile(bufsize=1)
            instruct = f.readline().strip().split()
            if not instruct:
                continue
            f.write(self.inparser.parseInstruct(instruct)+'\n')
...
sever = Server()
parser = InstructParser(sever)
severApplication = SeverApplication(parser)
severApplication.connect()
